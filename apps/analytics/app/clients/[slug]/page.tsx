...
import { getClientByPlatformSlug } from '@/lib/social-bridge';
import {
...
export default async function ClientHome({ params }: Props) {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <section className="flex flex-wrap gap-3">
        <Link href={`/clients/${slug}/ask`} className="btn-primary">
          Ask the data
        </Link>
        <Link href={`/clients/${slug}/posts`} className="btn-secondary">
          Open post matrix
        </Link>
...
function Hero({ name, slug }: { name: string; slug: string }) {
  return (
    <section className="grain relative overflow-hidden rounded-lg border border-green-deep/10 bg-green-deep px-8 py-10 text-cream shadow-card">
      ...
      <span className="eyebrow cream">Client</span>
