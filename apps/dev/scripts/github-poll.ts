...
import { pollAllProjects } from '../lib/monitors/github';

async function main() {
  const result = await pollAllProjects();
  console.log(`✔ polled ${result.polled} project(s) (${result.skipped} skipped — no github_repo).`);
