/**
 * Read-only bridge into the social tool's `public.*` schema. The dev hub never
 * writes to public — see ../packages/db/ENTITY_OWNERSHIP.md.
 *
 * Returns null on any failure (missing schema, missing tables, no DB) so the
 * dashboard widget gracefully no-ops instead of bringing the whole page down.
 */
import { q } from './db';
