/**
 * Direct Postgres access for the dev hub. Self-contained on purpose so the
 * app can deploy as a standalone Vercel project (no workspace deps to resolve).
 *
...
function makePool(): Pool {
  ...
  if (!cs) {
    throw new Error(
      'PG_URL is not set. Add it to apps/dev/.env.local — see .env.example.'
    );
  }
  return new Pool({
    connectionString: cs,
