...
import { exchangeCodeForToken, listManagedPages } from '@/lib/analytics/meta';
import { upsertPlatformAccount } from '@/lib/analytics/queries';
...
export async function GET(req: Request) {
  ...
  if (!code || !clientId) {
    return NextResponse.redirect(
      `${url.origin}/analytics/integrations?error=${encodeURIComponent('Missing code or state')}`
    );
  }
  ...
  if (!userToken) {
    return NextResponse.redirect(
      `${url.origin}/analytics/integrations?error=${encodeURIComponent('Token exchange failed')}`
    );
  }
  ...
  return NextResponse.redirect(
    `${url.origin}/analytics/integrations?connected=${connected}&pages=${pages.length}`
  );
}
