...
import { listStalledLeads } from '@/lib/crm/queries';
import { upsertNotification } from '@/lib/crm/notifications';
...
async function run(req: Request) {
  ...
  for (const lead of stalled) {
    ...
    await upsertNotification({
      ...
      link_url: crmBase ? `${crmBase}/leads/${lead.id}` : `/crm/leads/${lead.id}`,
