...
import { listClientsForAnalytics, getClientById } from '@/lib/analytics/social-bridge';
import { Markdown } from '@/components/analytics/markdown';
import { formatNumber, formatPct, snippet } from '@/lib/analytics/format';
...
function InsightPanel({
  ...
  return (
    <div className="panel p-6">
      ...
      {clientSlug && (
        <Link
          href={`/clients/${clientSlug}/performance`}
      ...
      {clients.length === 0 ? (
        ...
        <ul className="mt-3 flex flex-col gap-1.5">
          {clients.slice(0, 12).map(c => (
            <li key={c.id}>
              <Link
                href={`/clients/${c.slug}/performance`}
  ...
  return (
    <div className="grid gap-3 rounded-md border border-dashed border-cream-dk bg-white/60 p-8 text-center">
      ...
      <div className="mt-2 flex flex-wrap justify-center gap-3">
        <Link href="/analytics/integrations" className="btn-primary">
          <Plug size={14} /> Connect a platform
        </Link>
        <Link href="/analytics/import" className="btn-secondary">
