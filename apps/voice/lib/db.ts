/**
 * Direct Postgres access for the voice gateway. Self-contained on purpose so
 * the app can deploy as a standalone Vercel project (no workspace deps).
 *
...
function makePool(): Pool {
  ...
  if (!cs) {
    throw new Error(
      'PG_URL is not set. Add it to apps/voice/.env.local — see .env.example.'
    );
  }
  return new Pool({
    connectionString: cs,
